...
def export_dpan(bck_path,out):
    ...
    total=128+48+len(data); p=bytearray(total);p[:4]=b'DPAN';struct.pack_into('<HHI',p,4,1,128,total);struct.pack_into('<III',p,16,1,joints,frame_max);struct.pack_into('<II',p,32,128,48);entry=128;struct.pack_into('<I',p,entry,7);struct.pack_into('<IIII',p,entry+8,frame_max,samples,joints,0);struct.pack_into('<II',p,entry+24,176,len(data));p[176:]=data;struct.pack_into('<I',p,12,crc(p));(out/'title_logo.dpan').write_bytes(p)
