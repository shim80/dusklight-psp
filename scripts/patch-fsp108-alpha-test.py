...
def patch(path: Path) -> tuple[int, int]:
    ...
    for index in range(submesh_count):
        ...
        if bucket not in (0, BUCKET_ALPHA_TEST):
            raise ValueError(f'material {material_id} has unexpected bucket {bucket}')
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Restore audited F_SP108 foliage materials to DPRM alpha-test bucket.')
    ...
    try:
        ...
        return 1
    print('FSP108_ALPHA_TEST_PATCH_OK '
